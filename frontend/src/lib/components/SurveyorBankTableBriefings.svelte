<script lang="ts">
    ...
    // Refs for synced top scrollbar
    let tableContainer: HTMLDivElement;
    let tableEl: HTMLTableElement;
    let topScroll: HTMLDivElement;
    let topScrollInner: HTMLDivElement;
    let contentWidth = 0;
    let isSyncing = false;
    ...
    function updateContentWidth() {
        if (!tableEl) return;
        contentWidth = tableEl.scrollWidth;
    ...
    function handleTopScroll() {
        if (!tableContainer || !topScroll || isSyncing) return;
        isSyncing = true;
    ...
    // Recalculate after DOM updates that change table width
    $: (async () => {
        // Track dependencies to trigger recalculation
        displaySurveyors;
        sortKey; sortDirection; searchText;
        await tick();
        updateContentWidth();
    })();
...
<div class="surveyor-bank-container">
    ...
    {:else}
        ...
        {:else}
            <div class="top-scroll" bind:this={topScroll} on:scroll={handleTopScroll}>
                <div class="top-scroll-inner" bind:this={topScrollInner} style="width: {contentWidth}px;"></div>
            </div>
            <div class="table-container" bind:this={tableContainer} on:scroll={handleTableScroll}>
                <table class="surveyors-table" bind:this={tableEl}>
                    <thead>
                        <tr>
                            <th rowspan="2">
...
<style>
    ...
    .top-scroll-inner {
        height: 1px; /* minimal height; scrollbar shows due to overflow */
    }
